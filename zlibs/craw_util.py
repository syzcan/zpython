...
# 请求数据，最多3次
def request_get(craw_url,headers=None,cookies=None):
	if headers == None:
		headers = {}
	...
	while True:
		try:
			...
			if count>3:
				raise Exception("false to request["+craw_url+"] "+str(e))
				break
			count += 1
'''
解析列表页面
'''
def craw_list(craw_url,craw_item,craw_next,extFields,headers=None,cookies=None):
	response = request_get(craw_url,headers=headers,cookies=cookies)
	...
	doc = pq(content)
 	...
 	if craw_next !=None and craw_next != "":
 		if craw_next['rule_ext_type'].startswith('url'):
 			rule_ext_reg = ''
 			m = re.compile(r'\[(.*)\]').search(craw_next['rule_ext_type'])
 			if m:
...
解析详情页面
'''
def craw_detail(craw_url,extFields,headers=None,cookies=None):
	response = request_get(craw_url,headers=headers,cookies=cookies)
	content = response.text
	doc = pq(content)
...
def parse_ext(craw_url,item,extFields):
	...
	for ext in extFields:
		...
		if items.length > 0:
			...
			rule_ext_reg = ''
 			m = re.compile(r'\[(.*)\]').search(rule_ext_type)
			if m:
